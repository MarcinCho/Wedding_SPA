...
<template>
  <div class="w-full max-w-md mx-auto p-4 bg-gray-800 rounded-lg shadow-xl">
    
    <!-- Upload Area -->
    <div 
      v-if="!selectedFile && uploadStatus !== 'success'"
      class="border-2 border-dashed border-gray-600 rounded-lg p-8 text-center hover:border-blue-500 transition-colors cursor-pointer relative"
      @dragover.prevent
      @drop="handleDrop"
    >
      <input 
      ...
      <div class="space-y-2 pointer-events-none">
        ...
        <p class="text-gray-300 font-medium">Tap to take photo</p>
    ...
    <div v-if="isCompressing" class="text-center p-8">
      ...
      <p class="text-gray-300">Compressing image...</p>
    </div>
    ...
    <div v-if="selectedFile && !isCompressing" class="space-y-4">
      ...
      <div class="space-y-4">
        <div v-if="isUploading">
          ...
          <p class="text-center text-sm text-gray-400 mt-2">Uploading...</p>
        </div>

        <button 
          v-else
          @click="upload"
          class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg shadow-lg transition-transform active:scale-95 flex justify-center items-center gap-2"
        >
    ...
    <div v-if="uploadStatus === 'success'" class="text-center p-8 space-y-4">
      <div class="inline-flex items-center justify-center h-16 w-16 rounded-full bg-green-900/30 text-green-400 mb-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
      </div>
      <h3 class="text-xl font-bold text-white">Upload Successful!</h3>
      <p class="text-gray-400">Your photo has been safely stored.</p>
      <button 
        @click="resetState" 
        class="mt-4 text-blue-400 hover:text-blue-300 font-medium hover:underline"
      >
